...
public class SnakePanel extends JPanel implements KeyListener{
	...
	public SnakePanel(){
        ...
        SnakeNode tail=new SnakeNode(GameContants.GAME_BOARD_WIDTH/2,( GameContants.GAME_BOARD_HEIGHT/2)+GameContants.MY_SNAKE_WIDTH_HEIGHT);
	...
	private void drawFood(Graphics g) {
		...
		g.fillRect(mySnake.getFood().getX(), mySnake.getFood().getY(), GameContants.MY_SNAKE_WIDTH_HEIGHT, GameContants.MY_SNAKE_WIDTH_HEIGHT);
...
//		System.out.println("snake length is"+mySnake.getCompleSnake().size());
		...
		for(int a=0;a<mySnake.getCompleSnake().size(); a++) {
			...
			g.fillRect(node.getX(), node.getY(), GameContants.MY_SNAKE_WIDTH_HEIGHT, GameContants.MY_SNAKE_WIDTH_HEIGHT);
